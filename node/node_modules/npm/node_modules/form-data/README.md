...
You can provider custom options, such as `maxDataSize`:
...
Form-Data can recognize and fetch all the required information from common types of streams (```fs.readStream```, ```http.response``` and ```mikeal's request```), for some other types of streams you'd need to provider "file"-related information manually:
